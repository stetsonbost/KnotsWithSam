...
--since we seem to be using this again, we should stick it in a module before the final copy
firstList :: Int -> [[Int]]
...
secondList :: Int -> [[[Int]]]
secondList n = delete x (foldr (\x acc -> [[1,1,2,1,1],x]:acc) [] (firstList n))
	where x = [[1,1,2,1,1],[1,1,2,1,1]]
